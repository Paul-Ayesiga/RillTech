...
class SubscriptionController extends Controller
{
    ...
    public function checkout(Request $request)
    {
        ...
        try {
            ...
            // Extend session lifetime for checkout process
            config(['session.lifetime' => 240]); // 4 hours

            // Store user ID in session for recovery
            session(['checkout_user_id' => $user->id]);

            // Create checkout session with proper URLs
            $successUrl = route('subscription.success', [], true) . '?session_id={CHECKOUT_SESSION_ID}';
    ...
    public function success(Request $request)
    {
        ...
        try {
            // Use the recovery service to handle the complete checkout success flow
            $result = $this->recoveryService->handleCheckoutSuccess($sessionId);

            if ($result['success']) {
                // Clear checkout session data
                session()->forget('checkout_user_id');

                // Set success flag and redirect to dashboard
                session(['subscription_success' => true]);

                $message = $result['message'];
                if ($result['user_recovered']) {
                    $message .= ' You have been automatically logged back in.';
                }

                return redirect()->route('dashboard')
                    ->with('success', $message);
            } else {
                Log::warning('Checkout success flow failed', [
            ...
            Log::error('Error handling checkout success: ' . $e->getMessage(), [
                'session_id' => $sessionId,
